...
//        [actionButton setTitle:title forState:UIControlStateNormal];
        ...
        titleLabel.textColor = BaseMenuActionTextColor(self.style);
        titleLabel.font = [UIFont systemFontOfSize:14];
        titleLabel.text = title;
        [actionButton addSubview:titleLabel];
        [titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(actionButton.mas_top).offset(5);
...
- (void)layoutSubviews
{
    ...
    if (self.titleLabel) {
        self.titleLabel.frame = (CGRect){CGPointMake(0, height), CGSizeMake(self.bounds.size.width, 30)};
    ...
    for (int i=0; i<self.actionButtons.count; i++) {
        ...
        button.frame = (CGRect){CGPointMake(i * self.bounds.size.width / 2, btn_y), CGSizeMake(self.bounds.size.width / self.actionButtons.count, 55)};
        if (i == 0) {
            height += 44;
        }
    }
