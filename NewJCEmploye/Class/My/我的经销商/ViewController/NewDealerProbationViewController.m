//
//  NewDealerProbationViewController.m
//  NewJCEmploye
//
...
@interface NewDealerProbationViewController ()<SelectItemsDelegate,SelectProbationCustomDelegate>
...
@implementation NewDealerProbationViewController
...
-(void)addViews {
    ...
    _needView.button.tag = 4;
    [_needView.button addTarget:self action:@selector(buttonAction:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [lineView2 mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.equalTo(self.view);
        make.top.equalTo(_needView.mas_bottom).offset(5);
        make.height.equalTo(@(5));
    }];
...
-(void)test2 {
    ...
    JCModel_dealFranchiserProbationMBase *base = [[JCModel_dealFranchiserProbationMBase alloc]init];
    ...
    JCModel_dealFranchiserProbationMProbation *probation = [[JCModel_dealFranchiserProbationMProbation alloc]init];
    
    
    //填试用客户信息 以及 业务员ID 以及备注信息
    probation.preid = _listData.listIdentifier;

    NSString *str1 = [JCTokenManager getUserID];
    probation.uid = str1.doubleValue;
    probation.franchiser =str1;
    NSString *str = [JCTokenManager getUID];
    ...
    if (_isNeedOutput == YES) {
        ...
        probation.probationCosts = repertoryItemArray;
    }else {
    ...
    NSURLSessionDataTask *task = [manager dataTaskWithRequest:request completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
        if (!error) {
            ...
            if ([result  isEqual: @"success"] ) {
                ...
                [self.delegate newFProbationSuccess];
